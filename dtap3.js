 ...
 if (doses0[15]>=birthday) {doses.push(doses0[15]);} 
  ...
  else
  {
    ...
    var birthday = new Date(document.getElementById("bday").value);       
      ...
      var birthday1 = new Date(yearB + 1, monthB, dayB-4);    
    ...
    function isGreaterThanDose2(x,birthday,date3){
      var date3 = new Date(fDoses1[0]);  
    ...
    if (!isNaN(validDoses[2])) {
      ...
      document.getElementById("intervalDtap").innerHTML =
        "<li><b>Valid Dose 1 to Valid Dose 2: </b>"+Math.ceil((validDoses[1]-validDoses[0])/ (1000 * 3600 * 24))+" days</li>"+
          "<li><b>Valid Dose 2 to Valid Dose 3: </b>"+Math.ceil((validDoses[2]-validDoses[1])/ (1000 * 3600 * 24)/30)+" Months</li>";  
