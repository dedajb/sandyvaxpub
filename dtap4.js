function DTaP4(){  
  ...
  var startDoses = [];
  startDoses.push(dose1,dose2,dose3,dose4,dose5,dose6,dose7,dose8,dose9);
 ...
 if(!isNaN(dose6)){doses0.push(dose6);}  
  ...
  else
  {
    ...
    function isGreaterThanDose3(x,birthday,date3){
      ...
      var addDays4 = new Date(yearB + 4, monthB, dayB-4);
    ...
    else
    {
      ...
      if(isNaN(validDoses[2])){validDoses[2]="No Dose";} else {validDoses[2]=validDoses[2].toDateString()}
      if(isNaN(validDoses[3])){validDoses[3]="No Dose";} else {validDoses[3]=validDoses[3].toDateString()} 
      
      document.getElementById("intervalDtap").innerHTML =
        "<b>Non-Compliant Student: Needs additional dose(s)";        
      
      document.getElementById("resultDtap").innerHTML =
        "<b style='color:red;'>DTaP Status: Non-Compliant</b>"+"<li><i>Valid Dose 1: "+validDoses[0]+"</li>"+
          "<li><i>Valid Dose 2: "+validDoses[1]+"</li>"+
            "<li><i>Valid Dose 3: "+validDoses[2]+"</li>"+"<li><i>Valid Dose 4: "+validDoses[3]+"</li>";
    }
